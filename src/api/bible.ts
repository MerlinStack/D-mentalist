...
const STOP_WORDS = new Set([
  ...
  'from','by','about','against','between','into','through','god','lord','said',
  'unto','thee','thou','thy','ye','him','them','us','me','mine','thine',
])
...
export async function fetchMultipleVerses(references: string[], translation = 'kjv'): Promise<Verse[]> {
...
export async function searchByKeyword(query: string, translation = 'kjv'): Promise<Verse[]> {
  ...
  const res = await fetch(`${BASE_URL}/?search=${encodeURIComponent(searchTerm)}&translation=${translation}`)
  if (!res.ok) return []
...
export async function getChapter(book: string, chapter: number, translation = 'kjv'): Promise<Verse[]> {
  const res = await fetch(`${BASE_URL}/${encodeURIComponent(book)}+${chapter}?translation=${translation}`)
  if (!res.ok) return []
