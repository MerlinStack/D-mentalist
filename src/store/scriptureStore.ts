...
interface ScriptureState {
  ...
  setDetectionResult: (r: { verse: Verse | null; book: string | null; chapter: number | null; confidence: number; source: string | null }) => void
  ...
  persist(
    (set, get) => ({
      ...
      loadChapter: async (book, chapter) => {
        ...
        const apiCode = translation === 'KJV' ? 'kjv' :
          translation === 'NIV' ? 'web' :
          translation === 'ESV' ? 'darby' : 'kjv'
