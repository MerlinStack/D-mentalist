...
export default function VerseCard({ verse, query, index }) {
  const { setActiveVerse, activeVerse } = useScriptureStore()
  // Swapped openAndProject to projectVerse for the unified screen + browser window projection pipeline
  const { projectVerse, addToQueue } = useProjection()
  const isActive = activeVerse?.ref === verse.ref

  const confidence = verse.aiData?.confidence || (verse.relevance > 0.8 ? 'high' : verse.relevance > 0.5 ? 'medium' : 'low')
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-2">
        ...
        <div className="flex items-center gap-1">
          {/* Main Action Vector: Maps to the unified layout-swap presentation orchestrator hook */}
          <button
            onClick={(e) => { 
              e.stopPropagation(); 
              projectVerse(verse); // Swaps local operator UI view & projects to external displays simultaneously
            }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); addToQueue(verse) }}
