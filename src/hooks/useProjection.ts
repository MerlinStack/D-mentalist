...
export function useProjection() {
  ...
  const projectVerse = useCallback((verse: Verse) => {
  ...
  const updateTheme = useCallback((theme: 'dark' | 'light' | 'warm') => {
    store.setTheme(theme)
    channelRef.current?.postMessage({ type: 'SET_THEME', theme })
  }, [store])

  const updateFontSize = useCallback((fontSize: 'medium' | 'large' | 'xlarge') => {
