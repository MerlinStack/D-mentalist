  ...
  const search = useCallback(async (q) => {
    ...
    try {
      ...
      if (mode === 'Theme') {
        ...
        store.setResults([])
        store.setSearchError('No verses found for that theme. Try a single word like "faith" or "forgiveness".')
        store.setSearching(false)
        return
      }
      ...
      if (mode === 'Quote') {
        ...
        if (keywords.length > 0) {
          for (const kw of keywords.slice(0, 2)) {
            ...
            if (verses.length > 0) {
              ...
              const filtered = otherKeywords.length > 0
                ? verses.filter(v =>
        ...
        store.setResults([])
        store.setSearchError('No verses found. Try using key words like "loved", "shepherd", or "faith".')
        store.setSearching(false)
        return
      }
